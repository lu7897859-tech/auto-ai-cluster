...
function mdToHtml(content) {
  ...
  h = h.replace(/^\|(.+)\|$/gm, m => {
    ...
    if (c.every(x => /^[-:\s]+$/.test(x.trim()))) return '';
...
// Generate blog #19 HTML
const f = '19-observability-dashboard-grafana-2c4g.md';
const content = fs.readFileSync(path.join(BLOG, f), 'utf8');
const titleMatch = content.match(/^# (.+)/m);
const title = titleMatch ? titleMatch[1].trim() : 'Untitled';
const date = '2026-07-26';
const desc = 'Build a unified Grafana observability dashboard for your 2C4G AI cluster. Metrics, logs, and alerts in one view — zero additional RAM.';
...
<body>
<nav><a href="https://lu7897859-tech.github.io/auto-ai-cluster/">Home</a> &middot; <a href="https://lu7897859-tech.github.io/auto-ai-cluster/docs/blog/index.html">Blog</a></nav>

<h1>${title}</h1>
<p><small>Published ${date} &middot; 10 min read</small></p>
...
fs.writeFileSync(path.join(BLOG, '19-observability-dashboard-grafana-2c4g.html'), html, 'utf8');
console.log('Generated HTML for blog #19');
...
const entries = allMd.map(f => {
  ...
  const t = c.match(/^# (.+)/m);
  const title2 = t ? t[1].trim() : f.replace('.md','');
  const d = c.match(/^\*\*(\d{4}-\d{2}-\d{2})\*\*/m);
  const date2 = d ? d[1] : '2026-07-16';
  const paras = c.split('\n\n').filter(l => l.length > 30 && !l.startsWith('>') && !l.startsWith('#') && !l.startsWith('*'));
  const desc2 = paras.length > 0 ? paras[0].trim().substring(0, 120) : title2;
  const num = f.match(/^(\d+)/);
  return { title: title2, date: date2, desc: desc2, htmlName: f.replace('.md', '.html'), sortKey: num ? parseInt(num[1]) : 99 };
...
console.log('feed.xml (' + feed.length + ' bytes)');
