...
// Process blog 17
const f = '17-loki-promtail-centralized-logging-2c4g.md';
const content = fs.readFileSync(path.join(BLOG, f), 'utf8');
const titleMatch = content.match(/^# (.+)/m);
const title = titleMatch ? titleMatch[1].trim() : 'Untitled';
const date = '2026-07-25';
const desc = 'Run Grafana Loki + Promtail for centralized logging on a 2C4G VPS under 256 MB RAM. Docker log aggregation with LogQL and Grafana.';
...
fs.writeFileSync(path.join(BLOG, '17-loki-promtail-centralized-logging-2c4g.html'), html, 'utf8');
console.log('Generated HTML for blog #17');
...
const entries = allMd.map(f => {
  ...
  const desc2 = descP ? descP.trim().substring(0, 120) : title2;
  const num = f.match(/^(\d+)/);
